    ...
    class Generator
      ...
      class << self
        ...
        def process_path(path, root, config, settings)
          ...
          return {action: :skipped, path: path, reason: :not_tagged} unless tagged_by(klass.swagger_config[:tags])
        ...
        def get_route_path_apis(path, route, klass, settings, config)
          ...
          return {apis: apis, models: models, nickname: nil} unless tagged_by(operation.delete(:tags))
